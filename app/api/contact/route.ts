...
export async function POST(request: Request) {
  ...
  if (isNonEmptyString(data.sector)) pushField("sector", data.sector);
  if (Array.isArray(data.services) && data.services.length > 0) {
    pushField("services", data.services.join(";"));
  }
  if (isNonEmptyString(data.timing)) pushField("timing", data.timing);
  if (isNonEmptyString(data.budget)) pushField("budget", data.budget);
